---
id: l5-metallurgy-casting-basics
title: 'Metal Casting: Sand Casting and Mold Making'
category: L5_civilization_memory
subtopic: metallurgy
tags:
- metallurgy
- casting
- molds
- foundry
region_relevance:
- global
summary: Sand casting technique for reproducing metal parts using reusable patterns
  and sand molds.
steps:
- Create a pattern of the desired part from wood or existing object.
- Pack greensand (sand + clay + water) around the pattern in a two-part flask (cope
  and drag).
- Remove the pattern, cut gates and risers for metal flow and air escape.
- Melt metal in a crucible furnace and pour into the mold cavity.
- Allow to cool, break out casting, and finish by filing and grinding.
warnings:
- Molten metal is extremely dangerous—wear full protective gear.
- Moisture in the mold causes steam explosions—dry sand thoroughly.
related_entries:
- l5-metallurgy-basics
- l5-metallurgy-copper-working
- l5-metallurgy-iron-smelting
sources:
- usgs-mineral-education
- mit-ocw-mechanical-engineering
audit_status: verified
last_verified: '2026-02-19'
confidence: high
offline_assets: []
---

## Overview
Sand casting technique for reproducing metal parts using reusable patterns and sand molds.

## Step-by-step
1. Create a pattern of the desired part from wood or existing object.
2. Pack greensand (sand + clay + water) around the pattern in a two-part flask (cope and drag).
3. Remove the pattern, cut gates and risers for metal flow and air escape.
4. Melt metal in a crucible furnace and pour into the mold cavity.
5. Allow to cool, break out casting, and finish by filing and grinding.

## Warnings
- Molten metal is extremely dangerous—wear full protective gear.
- Moisture in the mold causes steam explosions—dry sand thoroughly.
