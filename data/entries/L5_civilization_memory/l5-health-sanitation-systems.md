---
id: l5-health-sanitation-systems
title: 'Sanitation Systems: Latrine Design and Waste Management'
category: L5_civilization_memory
subtopic: public_health
tags:
- public_health
- sanitation
- latrine
- waste
region_relevance:
- global
summary: Designing and building sanitation systems to prevent disease from human waste.
steps:
- Site latrines 30+ meters from water sources and downhill from wells.
- Dig pit latrines at least 2 meters deep with stable, lined walls.
- Build a slab cover with a hole and a lid to reduce flies and odor.
- Add ash, lime, or soil after each use to accelerate decomposition.
- When pits are 0.5m from full, cap with soil and dig a new pit.
warnings:
- 'Fecal contamination of water is the #1 killer in post-collapse scenarios.'
- Never use fresh human waste as fertilizer—compost for 1+ year first.
related_entries:
- l5-health-disease-prevention
- l5-health-water-infrastructure
- l5-governance-principles
sources:
- who-household-water-treatment
- cdc-water-emergency
audit_status: verified
last_verified: '2026-02-19'
confidence: high
offline_assets: []
---

## Overview
Designing and building sanitation systems to prevent disease from human waste.

## Step-by-step
1. Site latrines 30+ meters from water sources and downhill from wells.
2. Dig pit latrines at least 2 meters deep with stable, lined walls.
3. Build a slab cover with a hole and a lid to reduce flies and odor.
4. Add ash, lime, or soil after each use to accelerate decomposition.
5. When pits are 0.5m from full, cap with soil and dig a new pit.

## Warnings
- Fecal contamination of water is the #1 killer in post-collapse scenarios.
- Never use fresh human waste as fertilizer—compost for 1+ year first.
