---
id: l1-survival-winter-comprehensive
title: "Winter Survival"
category: L1_immediate_survival
subtopic: environment
tags:
- winter
- cold
- snow
- hypothermia
region_relevance:
- temperate
- boreal
confidence: high
sources:
- nols-wilderness-guide
related_entries:
- l1-medical-hypothermia
- l1-survival-arctic-basics
last_verified: '2026-02-18'
summary: Winter survival in temperate climates (0 to -20F). Hypothermia and frostbite are main threats. Layered clothing, insulated shelter, fire, dry socks. Water sources freeze - melt snow. Travel is
  exhausting in deep snow. Conserve energy.
warnings:
- Wet clothing causes rapid hypothermia even in mild cold (40F)
- Snow blindness (UV damage to eyes) causes temporary blindness - wear sunglasses or improvise eye protection
- Breaking through ice into water is often fatal - test ice thickness, avoid if possible
steps:
- 'CLOTHING LAYERS: Base wicking layer, insulation layer, windproof shell. Avoid cotton. Wool and synthetic retain warmth when wet. Hat, gloves, warm boots, extra socks.'
- 'SHELTER: Windproof and insulated. Snow provides excellent insulation (pile snow around tent or shelter walls). Small shelters warm faster with body heat.'
- 'FIRE: Essential. Gather dry wood (standing dead trees, inner bark). Build fire on platform (prevents melting into snow and extinguishing). Maintain fire constantly - restarting is hard.'
- 'WATER: Melt snow or ice near fire. Do not eat snow (cools body). Dehydration is common in winter (cold air is dry).'
- 'TRAVEL: Deep snow is exhausting. Use snowshoes or skis if available. Travel on ridges (less snow). Avoid avalanche terrain (see entry).'
- 'FROSTBITE: Watch for numb fingers, toes, nose, ears. Rewarm gradually (body heat, warm water). Do not rub (damages tissue).'
---

# Winter Survival

## Clothing Layers
- Base: wicking layer (wool or synthetic)
   -  keeps skin dry even when sweating
- Mid: insulation (wool, fleece, down)
- Outer: windproof and waterproof shell
- NEVER wear cotton  -  kills at 40F wet
- Hat, gloves, warm boots, extra socks

## Shelter
- Windproof is more important than
  insulation against moderate cold
- Bank snow around shelter walls
  (snow is excellent insulator)
- Small shelters warm faster with body heat
- See shelter entries for construction

## Fire and Water
- Build fire on platform (prevents sinking
  into snow and extinguishing itself)
- Gather dry wood from standing dead trees
- Melt snow near fire  -  do not eat snow
  (lowers core temp, causes dehydration)
- Dehydration is common in winter  - 
  cold dry air pulls moisture out fast

## Frostbite Watch
- Check numb fingers, toes, nose, ears
- Early: pale or waxy skin, loss of feeling
- Rewarm gradually (body warmth, warm
  water)  -  do NOT rub (breaks tissue)
- Do not let frostbitten areas refreeze

## Winter Travel
- Deep snow is exhausting  -  5x more
  calories than normal walking
- Travel on ridges (less snow depth)
- Avoid avalanche terrain at all costs

## Common Kills
- Wet cotton at 40F (hypothermia)
- Snow blindness (UV from snow reflection)
   -  improvise eye protection (card slits)
- Falling through ice  -  test every step,
  avoid if unclear, carry ice picks
