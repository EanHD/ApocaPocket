---
id: l1-survival-desert-comprehensive
title: "Desert Survival"
category: L1_immediate_survival
subtopic: environment
tags:
- desert
- heat
- dehydration
- survival
region_relevance:
- desert
confidence: high
sources:
- us-army-fm-4-25-11
- nols-wilderness-guide
related_entries:
- l1-water-desert-sources
- l1-strategy-environment-profiles
last_verified: '2026-02-18'
summary: 'Desert survival priorities: water, shade, avoiding heat stress. Dehydration kills fast in desert heat (130F+). Travel at night, rest in shade by day. Conserve water - do not ration (drink when
  thirsty). Signal for rescue. Many desert hazards (snakes, scorpions, flash floods).'
warnings:
- Dehydration symptoms (dark urine, dizziness, confusion) mean you are already in danger
- Desert temperatures swing 40-50F between day and night - prepare for both extremes
- Flash floods kill more people in deserts than dehydration - avoid washes during rain
steps:
- 'WATER: Priority 1. Find water sources (see entry) or carry enough. Minimum 1 gallon per day (more if active). Do NOT ration water - drink when thirsty. Body needs water to cool itself.'
- 'SHADE: Avoid midday heat (10am-4pm). Rest in shade or improvised shelter (tarp, rock overhang, dig trench and cover). Travel early morning, evening, night.'
- 'CLOTHING: Cover all skin (sun protection, reduces sweat evaporation). Light-colored loose clothing. Head covering essential (40 percent heat loss from head).'
- 'SIGNALING: Open terrain = high visibility. Use mirror, fire, bright clothing. Stay visible (ridgelines, clearings).'
- 'HAZARDS: Snakes and scorpions hide in shade (shake out boots, check before sitting). Flash floods in washes (move to high ground if rain visible). Quicksand (rare but possible near dry lakes).'
---

# Desert Survival

## Water Is Everything
- You lose 1 liter per hour sweating
  in extreme desert heat
- 1 gallon minimum per day when active
- DO NOT ration water  -  drink when thirsty
  Body needs water to cool itself
- Dark urine = danger zone already

## Heat Management
- Rest from 10am to 4pm  -  find shade
- Travel dawn, dusk, or night only
- Shade: rock overhang, tarp, dig trench
  and cover with branches or tarp
- Deserts drop 40-50F at night  - 
  prepare for cold too

## Clothing Strategy
- Cover ALL skin (sun + sweat reduction)
- Light-colored loose clothing
- Head covering  -  40% heat loss from head
- Do NOT strip off clothing in heat
  (exposed skin burns and dehydrates faster)

## Find Water
- Rock crevices and shaded canyon walls
- Dry riverbeds: dig 1-3 feet in outside
  bend where water pooled last
- Morning dew on rocks/plants
- Barrel cactus (emergency source)
- Do not follow animals at night  - 
  they know water but travel far

## Desert Hazards
- Snakes and scorpions hide in shade  - 
  shake out boots before putting on,
  check before sitting on rocks/logs
- Flash floods in washes  -  move
  to high ground if rain visible anywhere
- Quicksand near dry lake beds (rare)

## Signaling
- Open terrain = high visibility
- Use mirror (day), fire (night)
- Bright colored material on high ground
- Stay visible  -  don't hide in vegetation
