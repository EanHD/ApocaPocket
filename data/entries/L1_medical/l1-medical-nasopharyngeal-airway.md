---
id: l1-medical-nasopharyngeal-airway
title: "NPA Airway Insertion"
category: L1_immediate_survival
subtopic: medical_airway
tags:
- airway
- NPA
- nasopharyngeal-airway
- breathing
- unconscious-patient
region_relevance:
- global
summary: Nasopharyngeal airway insertion technique including proper sizing (nose-to-earlobe measurement), lubrication, insertion method, complications management, and when NPA is preferred over oropharyngeal airway.
steps:
- Assess need for airway adjunct (unconscious, airway obstruction)
- Size NPA using nose-to-earlobe measurement
- Lubricate NPA thoroughly with water-soluble lubricant
- Insert along floor of nose (parallel to hard palate, NOT upward)
- Confirm placement (air movement, no bleeding)
- Secure and monitor continuously
warnings:
- NEVER use NPA if skull fracture suspected (can penetrate brain)
- Epistaxis (nosebleed) is common complication - usually minor
- Improper sizing can cause trauma or inadequate airway
- NPA does not protect against aspiration - patient still at risk
- Severe facial trauma may make NPA insertion impossible
related_entries:
- l1-medical-cpr-basics
- l1-medical-head-injury
- l1-medical-unconscious-patient
sources:
- American Heart Association BLS Guidelines
- ATLS Advanced Trauma Life Support 10th Edition
- Prehospital Trauma Life Support (PHTLS) 9th Edition
- Airway Management in Emergencies (Roberts & Hedges)
last_verified: '2026-02-19'
confidence: high
audit_status: verified
offline_assets: []
---

# NPA Airway Insertion

## What It Is / Use When

- Soft rubber tube through nose to hold airway open past the tongue
- Better tolerated than OPA  -  works with intact gag reflex
- Use when: semi-conscious (GCS 9-13), snoring breaths, clenched jaw
- Snoring = tongue blocking airway = NPA needed
- Allows bag-mask ventilation and suctioning with clenched jaw

## NEVER use NPA if...

- Suspected skull fracture  -  NPA can penetrate brain through fracture
- Battle's sign: bruising behind one or both ears after head injury
- Raccoon eyes: bilateral black eyes appearing after head injury
- CSF leak: clear watery fluid from nose or ear after head trauma
- Severe midface fractures (LeFort) or major facial trauma

## Sizing

- Measure from tip of nostril to earlobe  -  this determines length
- Diameter: largest that slides in without force (size of pinky finger)
- Adults: 7-8mm internal diameter is typical
- Choose the larger, less obstructed nostril (inspect both first)
- Too small = ineffective airway, too large = bleeding

## Insertion Steps

- Lubricate entire tube with water-soluble gel or water  -  never dry
- Bevel facing the septum (midline), angle parallel to floor of mouth
- Do NOT angle upward  -  goes into top of nasal cavity, causes trauma
- Insert with gentle rotating motion  -  do not force past resistance
- If firm resistance: try other nostril or smaller size
- Advance until flange rests flush at nostril opening
- Confirm placement: feel air movement through tube during exhalation

## After Insertion

- Tape flange to nose or cheek to prevent dislodgement
- Assess every 5-15 minutes: air moving, breathing improved
- Minor nosebleed is common  -  pinch soft nose if moderate bleeding
- Severe bleeding: leave tube in place, apply pressure 10 minutes
- If patient becomes alert enough: allow self-removal, monitor airway

## Alternatives

- No NPA available: jaw-thrust maneuver or recovery position
- Patient deeply unconscious (GCS 8 or less): OPA instead
- Patient apneic or deteriorating: upgrade to advanced airway (LMA)
- With bag-mask: NPA improves seal, air passes through if jaw clenched
