---
id: l1-strategy-neighborhood-mutual-aid
title: Neighborhood Aid Network
category: L1_immediate_survival
---
# Neighborhood Aid Network

## What This Covers
Build neighbor networks before disaster. Collective action
outperforms individual stockpiling every time.

## Before Disaster
- Know your 3 nearest neighbors by name.
- Exchange phone numbers and a backup contact.
- Establish a check-in spot (front yard, corner, park).
- Agree on a check-in time after any major event.

## Skills Inventory
- Find out who has: medical training, generator, tools.
- Identify: HAM radio operator, mechanic, nurse, farmer.
- Note: who has well water, large food stores, truck/trailer.
- Document this — write it down and share with the group.

## Vulnerable Neighbors
- Elderly living alone — check on them first.
- Single parents with young children.
- People with disabilities or medical needs.
- Non-English speakers who may miss alerts.
- These are your first priority in any emergency.

## Comms Without Power
- Knock on doors. Leave written notes.
- Pre-agree on a physical meeting spot.
- Use chalk on sidewalk for status messages.
- Post status note on your front door (OK / Need Help).

## Resource Sharing
- Establish clear, simple rules before you need them.
- Rotate scarce resources fairly — document distribution.
- Shared generator use: agree on schedule in advance.
- Food pooling: communal meals stretch supplies further.

## Security
- Groups deter opportunistic threats better than individuals.
- Establish a neighborhood watch rotation.
- Share information about unusual activity.
- Avoid advertising individual stockpile quantities.

## Lead Without Authority
- Suggest actions, don't demand them.
- Volunteer first — others will follow.
- Acknowledge contributions publicly.
- Avoid power struggles — focus on outcomes.

## Children's Roles
- Kids can carry messages between households.
- Light tasks: gathering firewood, filling containers.
- Assign normalcy activities — games, structured time.
- Children watching children frees adults for tasks.

## Conflict Prevention
- Agree on rules before resources are scarce.
- Written agreements prevent memory disputes.
- Designate one person as coordinator per block.
- Rotate leadership to prevent resentment.
